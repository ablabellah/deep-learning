...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    negative_indices = tf.compat.v1.where(tf.logical_and(
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    shared = KL.Lambda(lambda x: K.squeeze(K.squeeze(x, 3), 2),
                       output_shape=(None, None, 1024), name="pool_squeeze")(x)
    ...
    # Check if the second dimension is None
    def reshape_bbox(x):
        shape = tf.shape(x)
        return tf.cond(
            tf.equal(shape[1], None),
            lambda: KL.Reshape((-1, num_classes, 4), name="mrcnn_bbox")(x),
            lambda: KL.Reshape((shape[1], num_classes, 4), name="mrcnn_bbox")(x)
        )

    # Use the custom reshape_bbox function to handle dynamic shape
    mrcnn_bbox = KL.Lambda(reshape_bbox, output_shape=(None, num_classes, 4))(x)
